...
def test_admin_config_preserves_managed_env_source_contract(monkeypatch, tmp_path):
    ...
    env_file.write_text("MODEL=opencode/managed-model\n", encoding="utf-8")
    app = create_app(lifespan_enabled=False)
...
def test_admin_apply_writes_complete_managed_env_and_masks_preview(
    ...
    assert "OPENCODE_API_KEY=********" in body["env_preview"]
    env_file = tmp_path / ".fcc" / ".env"
    text = env_file.read_text("utf-8")
    assert "MODEL=opencode/test-model" in text
...
def test_admin_apply_preserves_hidden_diagnostics_and_smoke_values(
    ...
    env_file.write_text(
        "\n".join(
            [
                ...
                "FCC_SMOKE_MODEL_OPENCODE=opencode/smoke-model",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    response = _local_client(app).post(
        "/admin/api/config/apply",
        json={"values": {"MODEL": "opencode_go/test-model"}},
    )
    ...
    assert body["applied"] is True
    text = env_file.read_text("utf-8")
    assert "MODEL=opencode_go/test-model" in text
...
def test_admin_apply_preserves_existing_managed_values(monkeypatch, tmp_path):
    ...
    response = _local_client(app).post(
        "/admin/api/config/apply",
        json={"values": {"MODEL": "opencode/test-model"}},
    )
...
def test_admin_process_env_values_are_locked_and_not_written(monkeypatch, tmp_path):
    _set_home(monkeypatch, tmp_path)
    _clear_process_config(monkeypatch)
    monkeypatch.setenv("MODEL", "opencode_go/process-model")
    app = create_app(lifespan_enabled=False)
    ...
    response = _local_client(app).post(
        "/admin/api/config/apply",
        json={"values": {"MODEL": "opencode/managed-model"}},
    )

    assert response.status_code == 200
    env_file = tmp_path / ".fcc" / ".env"
    assert "opencode/managed-model" not in env_file.read_text("utf-8")
...
def test_admin_first_apply_migrates_repo_env(monkeypatch, tmp_path):
    ...
    config = _local_client(app).get("/admin/api/config").json()
    model_field = next(field for field in config["fields"] if field["key"] == "MODEL")
    assert model_field["value"] == "opencode/deepseek-chat"
    ...
    assert "MODEL=opencode/deepseek-chat" in managed_text
    assert "OPENCODE_API_KEY=oc-secret" in managed_text


def test_admin_local_provider_status_returns_404(monkeypatch, tmp_path):
    """Local-status endpoint was removed in the opencode-only migration."""
    _set_home(monkeypatch, tmp_path)
    _clear_process_config(monkeypatch)
    app = create_app(lifespan_enabled=False)
